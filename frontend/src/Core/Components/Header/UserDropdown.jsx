...
import UserSkelaton from "../Loadings/userSkelaton";
...
export default function UserDropdown() {
  ...
  const {isLoading: isLoagout, logout} = useLogout();
      ...
      >
        <div>
          <span className="block font-medium text-gray-700 text-theme-sm ">
            {`${user ? user?.firstName + " " + user?.lastName : "User"}`.trim()}
          </span>
        ...
        <DropdownItem
            ...
            disabled={isLoagout}
            className="flex items-center gap-3 px-3 py-2 font-medium text-gray-700 rounded-lg group text-theme-sm hover:bg-gray-100 hover:text-gray-700    mt-2"
          >
          <svg
